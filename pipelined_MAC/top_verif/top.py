...
async def test_mac(dut):
    ...
    for i in range(5,len(arr_out_corner)):
    	dut._log.info(f'OUTPUT {bin(arr_out_corner[i])}  model: {bin(arr_model_corner[i-5])} ')
    	
    	# ignoring the NaN outputs
    	exponent_bits = arr_model_corner[i-5] & 0b01111111111111111111111111111111;
    	if (exponent_bits <= 2139095040):
    		assert int(arr_model_corner[i-5]>>2) == int(arr_out_corner[i]>>2), f' {i-4} : Counter Output Mismatch, Expected = {bin(arr_model_corner[i-5])} {int(arr_model_corner[i-5])} DUT = {bin(arr_out_corner[i])} {int(arr_out_corner[i])}' 
